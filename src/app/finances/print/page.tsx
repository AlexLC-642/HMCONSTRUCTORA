...
export default async function FinancePrintPage({
	searchParams,
}: FinancePrintPageProps) {
	const user = await requirePermission("finanzas.ver");
	const params = await searchParams;
	const { selectedProject, expenses, payments, payables, summary } =
		await getFinanceWorkspace(user, params.projectId);
	const backHref = selectedProject
		? `/finances?projectId=${selectedProject.id}`
		: "/finances";
	...
	const supplierPaid = payables.reduce(
		(sum, payable) => sum + payable.paid.toNumber(),
		0,
	);
	const supplierPending = payables.reduce(
