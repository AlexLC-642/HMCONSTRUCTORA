      ...
      <section className="relative grid min-h-screen grid-rows-[auto_1fr_auto] px-6 py-8 sm:px-10 lg:grid-cols-[1.02fr_0.98fr] lg:grid-rows-[auto_1fr_auto] lg:px-16">
        <header className="login-brand-mark flex items-center gap-4 lg:col-span-2">
          <div className="relative grid size-[74px] shrink-0 place-items-center rounded-lg border border-[#cfd3cf] bg-white shadow-[0_12px_32px_rgba(20,25,27,0.14)] sm:size-[86px]">
            <Image alt="Logo del sistema" className="object-contain p-2.5" fill priority sizes="86px" src="/brand/logo.png" />
          </div>
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Control de obra</h1>
          </div>
        </header>

        <section className="flex max-w-2xl flex-col justify-center py-14 lg:py-0">
          <p className="login-kicker text-sm font-bold uppercase tracking-[0.34em] text-[var(--brand-red)]">Control de obra</p>
        ...
        <section className="flex items-center justify-center pb-16 lg:pb-0">
          <div className="login-card w-full max-w-[500px] rounded-xl border border-[#cfd3cf] bg-white/94 p-8 shadow-[0_30px_80px_rgba(30,36,38,0.16)] backdrop-blur-sm sm:p-10">
            <div className="mb-8 flex items-center gap-5">
              <div className="relative grid size-[82px] shrink-0 place-items-center rounded-xl border border-[#d7d9d6] bg-white shadow-[0_14px_34px_rgba(20,25,27,0.15)]">
                <Image alt="Logo del sistema" className="object-contain p-2.5" fill priority sizes="82px" src="/brand/logo.png" />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.32em] text-[var(--brand-red)]">Acceso autorizado</p>
                <h3 className="mt-3 text-3xl font-semibold tracking-[-0.02em]">Iniciar sesion</h3>
              </div>
            </div>
            ...
            <form action={loginAction} className="space-y-5">
              <label className="block">
                <span className="mb-2 block text-sm font-medium">Usuario corporativo</span>
              ...
              <div className="block">
                <label className="mb-2 block text-sm font-medium" htmlFor="login-password">Contraseña</label>
                <PasswordInput />
              </div>

              <button className="login-submit focus-ring h-14 w-full rounded-md bg-[var(--brand-red)] px-4 text-lg font-semibold text-white shadow-[0_18px_36px_rgba(200,32,47,0.24)]" type="submit">
                Entrar al sistema
              </button>
            </form>
            <PasskeyLogin />
