...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hasPermission(user.permissions, "avance.crear")) return apiError(403, "FORBIDDEN", "No tienes permiso para crear avances.");

  const { id: projectId } = await params;
  const body = offlineSyncSchema.safeParse(await request.json().catch(() => null));

  if (!body.success) {
    return apiError(422, "VALIDATION_ERROR", "La operacion offline no tiene el formato correcto.");
  }
  ...
  const operation = existing ?? await prisma.syncOperation.create({
    data: {
  ...
  try {
    const parsed = readDailyReportFormData(formDataFromRecord(body.data.formData));
    const report = await saveDailyReport(projectId, parsed, { userId: user.id });
    const result = { reportId: report.id, reportNumber: report.reportNumber, status: report.status };
    ...
    return NextResponse.json({ success: true, data: result, error: null, meta: { replayed: false } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "No se pudo sincronizar el informe.";
    await prisma.syncOperation.update({
      where: { id: operation.id },
      data: { status: "FAILED", lastError: message }
    });

    if (error instanceof Prisma.PrismaClientKnownRequestError || error instanceof z.ZodError) {
