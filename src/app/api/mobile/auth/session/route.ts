...
export async function POST(request: Request) {
	...
	if (isMobileSessionLimited(ipAddress)) {
		return NextResponse.json({ error: "Demasiados intentos. Inténtalo de nuevo." }, {
			status: 429,
	...
	const parsed = mobileTokenSchema.safeParse({ deviceToken: bearerToken(request) });
	if (!parsed.success) {
		return NextResponse.json({ error: "Credencial móvil inválida." }, { status: 401 });
	}
	...
	if (
		...
		return NextResponse.json({ error: "Este dispositivo debe vincularse nuevamente." }, {
			status: 401,
			headers: { "Cache-Control": "no-store" },
		});
	}
	...
	await recordAuditLog({
		...
		metadata: { deviceName: credential.deviceName, platform: credential.platform },
		ipAddress,
	});
	return response;
}
