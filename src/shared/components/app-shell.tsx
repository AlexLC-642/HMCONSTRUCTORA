...
function readSidebarPreference(storageKey: string, fallback: boolean) {
...
export function AppShell({
	...
	const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(() =>
		readSidebarPreference(sidebarStorageKey, initialSidebarCollapsed),
	);
	const [isMobileNavigationOpen, setIsMobileNavigationOpen] = useState(false);

	useLayoutEffect(() => {
		// biome-ignore lint/suspicious/noDocumentCookie: the server must receive this preference on the next route request to avoid a layout flash.
		document.cookie = `${sidebarCookieKey}=${isSidebarCollapsed}; Path=/; Max-Age=31536000; SameSite=Lax`;
	}, [isSidebarCollapsed, sidebarCookieKey]);
	...
	function toggleDesktopSidebar() {
		setIsSidebarCollapsed((current) => {
			...
			try {
				...
				// La cookie conserva la preferencia si el almacenamiento local falla.
			}
			return next;
		});
	}
	...
	const sidebarInitScript = `{try{var e=document.getElementById("app-shell");var v=localStorage.getItem(${JSON.stringify(sidebarStorageKey)});if(e&&v!==null)e.setAttribute("data-sidebar-collapsed",v==="true"?"true":"false")}catch(e){}}`;
	...
	return (
		<>
			...
			<InlineScript html={sidebarInitScript} />
		</>
	);
}
