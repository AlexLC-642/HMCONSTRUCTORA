...
export type DocumentPreviewVersionData = {
  id: string;
...
function normalizeVersion(version: unknown): DocumentPreviewVersionData | null {
  ...
  const fileName = typeof data.originalName === "string" ? data.originalName : "";
  ...
  return {
    ...
    publicUrl,
    createdAt: new Date((data.createdAt as string | number | Date | null | undefined) ?? Date.now()).toISOString(),
    notes: typeof data.notes === "string" ? data.notes : null,
    uploadedBy: rawUploadedBy && typeof rawUploadedBy === "object" ? {
...
export function buildDocumentPreview(
  ...
  const documentRecord = document && typeof document === "object" ? (document as Record<string, unknown>) : {};
  const projectRecord = project && typeof project === "object" ? (project as Record<string, unknown>) : (documentRecord.project && typeof documentRecord.project === "object" ? (documentRecord.project as Record<string, unknown>) : {});
  const categoryRecord = category && typeof category === "object" ? (category as Record<string, unknown>) : (documentRecord.category && typeof documentRecord.category === "object" ? (documentRecord.category as Record<string, unknown>) : {});
  const authorRecord = documentRecord.author && typeof documentRecord.author === "object" ? (documentRecord.author as Record<string, unknown>) : {};
  const approvedByRecord = documentRecord.approvedBy && typeof documentRecord.approvedBy === "object" ? (documentRecord.approvedBy as Record<string, unknown>) : {};
  const rawVersions: unknown[] = Array.isArray(documentRecord.versions) ? documentRecord.versions : [];

  const versions: DocumentPreviewVersionData[] = rawVersions
    .map((version) => normalizeVersion(version))
    .filter((version): version is DocumentPreviewVersionData => Boolean(version));

  const selectedVersion = previewVersion ? normalizeVersion(previewVersion) : versions[0] ?? null;

  return {
    id: String(documentRecord.id ?? ""),
    title: String(documentRecord.title ?? "Documento sin título"),
    description: typeof documentRecord.description === "string" ? documentRecord.description : null,
    ...
    createdAt: new Date((documentRecord.createdAt as string | number | Date | null | undefined) ?? Date.now()).toISOString(),
    updatedAt: new Date((documentRecord.updatedAt as string | number | Date | null | undefined) ?? Date.now()).toISOString(),
    ...
    versions,
    author: Object.keys(authorRecord).length > 0 ? { id: String(authorRecord.id ?? ""), name: String(authorRecord.name ?? "") } : null,
    approvedBy: Object.keys(approvedByRecord).length > 0 ? { id: String(approvedByRecord.id ?? ""), name: String(approvedByRecord.name ?? "") } : null
  };
}
