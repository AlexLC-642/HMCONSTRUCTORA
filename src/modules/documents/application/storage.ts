...
function detectMime(buffer: Buffer, extension: string) {
  ...
  if (buffer.subarray(4, 8).toString() === "ftyp") return extension === ".mov" ? "video/quicktime" : "video/mp4";
  if (buffer.subarray(0, 2).toString() === "PK" && [".docx", ".xlsx", ".pptx"].includes(extension)) return extensionMime[extension];
  if (buffer.subarray(0, 4).toString() === "AC10" && extension === ".dwg") return "image/vnd.dwg";
  if (buffer.subarray(0, 16).toString().includes("SECTION") && extension === ".dxf") return "image/vnd.dxf";
  return "";
}
...
async function validateAndReadFile(file: File, allowedMime: Record<string, string>, maxSize: number) {
  if (file.size <= 0) throw new Error("Selecciona un archivo valido.");
  if (file.size > maxSize) throw new Error(`El archivo supera el limite de ${Math.round(maxSize / (1024 * 1024))} MB.`);
  ...
  if (!detectedMime || detectedMime !== allowedMime[extension]) {
    throw new Error("El contenido del archivo no coincide con su tipo esperado.");
  }
...
async function writeStoredFile(storageKeyPrefix: string, buffer: Buffer, extension: string) {
  const fileName = `${randomUUID()}${extension}`;
  const storageKey = `${storageKeyPrefix}/${fileName}`;
  const outputDir = path.join(process.cwd(), "public", ...storageKeyPrefix.split("/"));
...
export async function storeWebsiteImageFile(mediaId: string, file: File) {
  const { buffer, extension, mimeType, checksum } = await validateAndReadFile(
    file,
    imageExtensionMime,
    MAX_WEBSITE_IMAGE_SIZE
  );
  const { fileName, storageKey, publicUrl } = await writeStoredFile(`uploads/website/${mediaId}`, buffer, extension);
