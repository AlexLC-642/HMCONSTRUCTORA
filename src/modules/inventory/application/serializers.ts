...
export function serializeMovement(
	movement: StockMovement & {
		...
		wasteReviewedBy?: { name: string } | null;
	},
) {
	const createdAtLabel = new Intl.DateTimeFormat("es-GT", {
		dateStyle: "short",
		timeStyle: "medium",
	})
	...
	return {
		...
		wasteReason: movement.wasteReason,
		wasteReviewStatus: movement.wasteReviewStatus,
		wasteReviewTriggers: Array.isArray(movement.wasteReviewTriggers)
			? movement.wasteReviewTriggers.filter(
					(item): item is string => typeof item === "string",
				)
			: [],
		wasteReviewNotes: movement.wasteReviewNotes,
		wasteReviewedAt: movement.wasteReviewedAt?.toISOString() ?? null,
		wasteReviewedBy: movement.wasteReviewedBy ?? null,
