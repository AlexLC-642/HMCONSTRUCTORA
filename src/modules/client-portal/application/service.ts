...
import { getApprovedBudgetChanges } from "@/modules/budgets/application/queries";
import { prisma } from "@/shared/lib/prisma";
import {
	clearPortalLookupAttempts,
	isPortalLookupRateLimited,
	recordFailedPortalLookup,
...
type PortalProject = Prisma.ProjectGetPayload<{
	include: {
		...
		budgets: {
			include: {
				versions: { include: { sections: { include: { lineItems: true } } } };
			};
		};
...
export async function getPortalShareStatus(
	projectId: string,
): Promise<PortalShareStatus | null> {
...
export async function getClientPortalByToken(
  ...
  `;
	...
	clearPortalLookupAttempts(ipAddress);
	...
	const budgetChanges = await getApprovedBudgetChanges(project.id);
	...
	return {
		...
		stats: {
			...
			budget:
				toNumber(approvedBudget?.grandTotal ?? project.baseBudget) +
				toNumber(budgetChanges.total),
