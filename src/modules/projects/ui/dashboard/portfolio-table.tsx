...
import { moneyCompact, percent, progressGap } from "@/shared/ui/charts/format";
...
export function PortfolioTable({
	...
	const rows = useMemo(() => {
		return [...projects].sort((a, b) => {
			const aValue =
				sortKey === "gap" ? a.realProgress - a.plannedProgress : a[sortKey];
			const bValue =
				sortKey === "gap" ? b.realProgress - b.plannedProgress : b[sortKey];
			const modifier = sortDesc ? -1 : 1;
			if (typeof aValue === "number" && typeof bValue === "number")
				return (aValue - bValue) * modifier;
			return (
				String(aValue ?? "").localeCompare(String(bValue ?? "")) * modifier
			);
		});
		...
		() => [
			{
				...
				cell: ({ row }) => (
					<div>
						...
						<p className="max-w-[18rem] truncate text-sm text-[var(--muted)]">
							{row.original.name}
						</p>
			...
			},
			{
				...
				header: "Cliente",
				cell: ({ row }) => (
					<span className="text-[var(--muted)]">{row.original.clientName}</span>
				),
			},
			{
			...
			},
			{
				...
				header: "Plan",
				cell: ({ row }) =>
					progressBar(row.original.plannedProgress, "bg-[var(--info)]"),
			},
			{
				id: "gap",
				header: "Estado frente al plan",
				accessorFn: (row: DashboardProjectRow) =>
					row.realProgress - row.plannedProgress,
				cell: ({ row }) => {
			...
			},
			{
				...
				cell: ({ row }) => (
					<span className="tabular-nums">
						{moneyCompact(row.original.budgetTotal)}
					</span>
				),
			},
			{
				...
				cell: ({ row }) => (
					<span className="tabular-nums">
						{moneyCompact(row.original.spent)}
					</span>
				),
			},
			{
				...
				cell: ({ row }) => (
					...
					>
						{moneyCompact(row.original.balance)}
					</span>
				),
			},
			{
			...
			},
			{
				...
				cell: ({ row }) => (
					<span
						className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${riskClass(row.original.risk)}`}
					>
						{row.original.risk}
					</span>
				),
			...
			},
			{
				...
				cell: ({ row }) => (
					<button
						aria-label={`Ver ${row.original.code}`}
		...
		<>
				...
				<table className="w-full min-w-[1120px] border-collapse text-sm">
					<thead>
						{table.getHeaderGroups().map((headerGroup) => (
							<tr
								className="border-b border-[var(--border)] bg-[#f4f3ef] text-left text-xs uppercase tracking-[0.04em] text-[var(--muted)]"
								key={headerGroup.id}
							>
					...
					<tbody>
						{table.getRowModel().rows.map((row) => (
							<tr
								className="border-b border-[var(--border)] bg-white/80 transition hover:bg-[#fff8f2]"
								key={row.id}
							>
			...
			<div className="grid gap-3 md:hidden">
				{rows.map((project) => {
					...
					return (
						<button
							className="rounded-2xl border border-[var(--border)] bg-white/80 p-4 text-left shadow-[0_10px_24px_rgba(37,48,51,0.05)]"
							key={project.id}
						...
						>
							<div className="flex items-start justify-between gap-3">
								...
								<span
									className={`inline-flex shrink-0 rounded-full border px-2.5 py-1 text-xs font-semibold ${riskClass(project.risk)}`}
								>
									{project.risk}
								</span>
							</div>
							<div className="mt-3">{progressBar(project.realProgress)}</div>
							<dl className="mt-3 grid grid-cols-3 gap-2 text-xs text-[var(--muted)]">
								<div>
									...
									<dd className="mt-1 font-bold tabular-nums text-[var(--foreground)]">
										{moneyCompact(project.budgetTotal)}
									</dd>
								</div>
								<div>
									<dt>Saldo</dt>
									<dd
										className={`mt-1 font-bold tabular-nums ${project.balance < 0 ? "text-[var(--danger)]" : "text-[var(--foreground)]"}`}
									>
										{moneyCompact(project.balance)}
									</dd>
								</div>
								<div>
									<dt>Frente al plan</dt>
									<dd
										className={`mt-1 font-bold tabular-nums ${gap < -5 ? "text-[var(--danger)]" : "text-[var(--success)]"}`}
									>
										{progressGap(gap)}
									</dd>
								</div>
