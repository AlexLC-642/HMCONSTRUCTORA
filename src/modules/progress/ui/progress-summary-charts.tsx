...
export function ProgressSummaryCharts({
	...
	const totalActivities = Math.max(
		0,
		completed + inProgress + pending + blocked,
	);
	const movedActivities = activities.filter(
				...
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						...
						<p className="mt-1 text-xs text-[var(--muted)]">
							Avance real contra la base planificada del proyecto.
						</p>
					</div>
					<span
						className={`rounded-full border px-3 py-1 text-xs font-semibold ${delta >= 0 ? "border-[#b7dfcc] bg-[#edf9f2] text-[var(--success)]" : "border-[#f1b6ba] bg-[#fff1f1] text-[var(--brand-red)]"}`}
					>
						{progressGap(delta)}
					</span>
				</div>
				...
				>
					<div className="flex items-start justify-between gap-3">
						<div>
							...
							<p className="mt-1 text-xs text-[var(--muted)]">
								{totalActivities} actividades base
							</p>
						</div>
					...
					<div className="grid gap-2">
						{statusRows.map((row) => (
							<div
								className="flex items-center justify-between rounded-xl border border-[var(--border)] bg-[#fbfaf6] px-3 py-2 text-sm"
								key={row.key}
							>
			...
			>
				<div className="flex flex-wrap items-center justify-between gap-3">
					<div>
						...
						<p className="mt-1 text-xs text-[var(--muted)]">
							{movedActivities} actividades con avance registrado o acumulado.
						</p>
					</div>
					<div className="flex flex-wrap gap-2 text-xs">
						<span className="rounded-full bg-[#edf9f2] px-2.5 py-1 font-semibold text-[var(--success)]">
							Real {pct(real)}
						</span>
						<span className="rounded-full bg-[#edf4ff] px-2.5 py-1 font-semibold text-[#2057c9]">
							Plan {pct(planned)}
						</span>
					</div>
				</div>
