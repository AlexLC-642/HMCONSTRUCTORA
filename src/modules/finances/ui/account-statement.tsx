...
function rowClass(expense: ExpenseRow) {
	const marker =
		`${expense.type ?? ""} ${expense.paymentMethod ?? ""} ${expense.description}`.toLowerCase();
	if (marker.includes("deposit") || marker.includes("abono"))
		return "deposit-row";
	return "expense-row";
}
					...
					<section key={phase}>
						<table aria-label={`${phase} estado de cuenta de gastos`}>
							...
							<tbody>
								{rows.length === 0 ? (
									<tr>
										<td className="center-cell" colSpan={7}>
											Sin compras ni gastos válidos registrados.
										</td>
									</tr>
								) : (
									rows.map((expense, index) => (
										<tr key={expense.id}>
											...
											<td className={`desc-cell ${rowClass(expense)}`}>
												{expense.description}
											</td>
											<td className="center-cell">{expense.vendor ?? "-"}</td>
											<td className="center-cell">
												{expense.quantity.gt(0)
													? numberFormatter.format(expense.quantity.toNumber())
													: "-"}
											</td>
								...
								<tr>
									<th colSpan={5} className="total-label">
										TOTAL {phase}
									</th>
									<th className="money-cell total-amount">
